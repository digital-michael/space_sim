...
// exec dispatches cmd to the appropriate domain handler and returns
// (done=true) when the REPL should exit.
func (r *REPL) exec(ctx context.Context, cmd commands.Cmd) (bool, error) {
	switch cmd.(type) {
	case commands.SetSpeed, commands.GetSpeed, commands.SetDataset, commands.GetDataset,
		commands.GetTime, commands.Stream, commands.Pause, commands.Resume, commands.Shutdown:
		return r.execSimCmds(ctx, cmd)
	case commands.SystemList, commands.SystemGet, commands.SystemLoad,
		commands.WindowGet, commands.WindowSize, commands.WindowMaximize,
		commands.WindowRestore, commands.WindowFullscreen:
		return r.execSystemCmds(ctx, cmd)
	case commands.CameraCenter, commands.CameraGet, commands.CameraOrient,
		commands.CameraPosition, commands.CameraTrack:
		return r.execCameraCmds(ctx, cmd)
	case commands.NavStop, commands.NavVelocity, commands.NavMove,
		commands.NavJump, commands.Orbit, commands.Sleep:
		return r.execNavCmds(ctx, cmd)
	case commands.PerfGet, commands.PerfSet, commands.HUD, commands.HUDList,
		commands.HUDCategory, commands.Labels, commands.Infra:
		return r.execPerfCmds(ctx, cmd)
	case commands.RecordStart, commands.RecordPause, commands.RecordStop, commands.RecordDelete:
		return r.execRecordCmds(ctx, cmd)
	case commands.SessionRegister, commands.SessionUnregister, commands.SessionList,
		commands.SessionKick, commands.SessionTeleport:
